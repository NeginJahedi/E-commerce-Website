...
def addproduct():
    if request.method == "POST":
        ...
        if not (len(variant_sizes) == len(variant_colors) == len(variant_stocks)):
            ...
            #  all image handling code was written with guidance from chatgpt
        if not image_file:
...
@app.route("/view-all-orders", methods=["GET"])
def view_all_orders():
    all_orders = db.execute("""
                            SELECT * FROM orders;
                            """)
    return render_template("viewallorders.html", orders = all_orders)
...
def view_order_details(order_id):
    ...
    ordered_items = db.execute ("""
        SELECT order_items.id, order_items.order_id, order_items.quantity, order_items.price,
        ...
        WHERE order_id = ?
        """, order_id )
    print(ordered_items[0]["product_id"])
    # product_name = db.execute
    # ("""
    #     SELECT name FROM products WHERE                     
    # """)
    return render_template("orderdetails.html", order = order, items = ordered_items)